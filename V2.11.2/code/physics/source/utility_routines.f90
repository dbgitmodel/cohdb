...
!
! Version - @(COHERENS)utility_routines.f90  V2.11.2
!
...
! Description -
!
! Generic routines - swap_data, tvd_limiter
!
...
!            relax_factor, string_replace, two_power, upper_case
!
...
INTERFACE tvd_limiter
   MODULE PROCEDURE tvd_limiter_0d, tvd_limiter_2d, tvd_limiter_3d
END INTERFACE
...
FUNCTION mult_index(ix,iy,matchzero)
!************************************************************************
!
! *mult_index* Return .TRUE. if 'ix' an integer multiple of 'iy'
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)utility_routines.f90  V2.11.2
!
! Description - if 'iy' equals zero, the result is .TRUE. if matchzero is
!               PRESENT and .TRUE., .FALSE. otherwise
!
!************************************************************************
!
!*  Arguments
...
FUNCTION tvd_limiter_0d(x)
!************************************************************************
!
! *tvd_limiter_0d* TVD limiting function at a specific grid point
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)utility_routines.f90  V2.1.2
!
! Description -
!
!************************************************************************
...
! Name         Type    Purpose
!------------------------------------------------------------------------------
!*x*           REAL    Argument for limiting factor
!*tvd_limiter* REAL    TVD limiting factor
!
!------------------------------------------------------------------------------
!


IF (iopt_adv_tvd.EQ.1) THEN
   tvd_limiter_0d = MAX(0.0,MIN(2.0*x,1.0),MIN(x,2.0))
ELSEIF (iopt_adv_tvd.EQ.2) THEN
   tvd_limiter_0d = (x+ABS(x))/(1.0+ABS(x))
ENDIF


RETURN

END FUNCTION tvd_limiter_0d

!========================================================================

FUNCTION tvd_limiter_2d(x,mask)
!************************************************************************
!
! *tvd_limiter_2d* TVD limiting function applied in 2-D
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)utility_routines.f90  V2.2
!
! Description -
!
!************************************************************************
...
LOGICAL, INTENT(IN), OPTIONAL, DIMENSION(LBOUND(x,1):UBOUND(x,1),&
                                       & LBOUND(x,2):UBOUND(x,2)) :: mask
REAL, DIMENSION(LBOUND(x,1):UBOUND(x,1),&
              & LBOUND(x,2):UBOUND(x,2)) :: tvd_limiter_2d

!
! Name         Type    Purpose
!------------------------------------------------------------------------------
!*x*           REAL    Argument for limiting factor
!*mask*        LOGICAL No evaluation is made where mask is .FALSE.
!*tvd_limiter* REAL    TVD limiting factor
!
!------------------------------------------------------------------------------
!
...
IF (iopt_adv_tvd.EQ.1) THEN
   IF (PRESENT(mask)) THEN
      WHERE (mask) 
         tvd_limiter_2d = MAX(0.0,MIN(2.0*x,1.0),MIN(x,2.0))
      END WHERE
   ELSE
      tvd_limiter_2d = MAX(0.0,MIN(2.0*x,1.0),MIN(x,2.0))
   ENDIF
ELSEIF (iopt_adv_tvd.EQ.2) THEN
   IF (PRESENT(mask)) THEN
      WHERE (mask) 
         tvd_limiter_2d = (x+ABS(x))/(1.0+ABS(x))
      END WHERE
   ELSE
      tvd_limiter_2d = (x+ABS(x))/(1.0+ABS(x))
   ENDIF
...
END FUNCTION tvd_limiter_2d

!========================================================================

FUNCTION tvd_limiter_3d(x,mask)
!************************************************************************
!
! *tvd_limiter_3d* TVD limiting function applied in 3-D
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)utility_routines.f90  V2.2
...
REAL, DIMENSION(LBOUND(x,1):UBOUND(x,1),&
              & LBOUND(x,2):UBOUND(x,2),&
              & LBOUND(x,3):UBOUND(x,3)) :: tvd_limiter_3d
...
IF (iopt_adv_tvd.EQ.1) THEN
   IF (PRESENT(mask)) THEN
      WHERE (mask) 
         tvd_limiter_3d = MAX(0.0,MIN(2.0*x,1.0),MIN(x,2.0))
      END WHERE
   ELSE
      tvd_limiter_3d = MAX(0.0,MIN(2.0*x,1.0),MIN(x,2.0))
   ENDIF
ELSEIF (iopt_adv_tvd.EQ.2) THEN
   IF (PRESENT(mask)) THEN
      WHERE (mask) 
         tvd_limiter_3d = (x+ABS(x))/(1.0+ABS(x))
      END WHERE
   ELSE
      tvd_limiter_3d = (x+ABS(x))/(1.0+ABS(x))
   ENDIF
...
! Author - Patrick Luyten
!
! Version - @(COHERENS)utility_routines.f90  V2.1.0
!
! Description -
!
!************************************************************************
