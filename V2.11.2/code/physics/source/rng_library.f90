...
!
! Version - @(COHERENS)rng_library.f90  V2.11
!
...
iranseed1 = default_seed1
iranseed2 = default_seed2
...
IF (iopt_rng_seed.EQ.1) THEN
   iranseed1 = default_seed1
   iranseed2 = default_seed2

!
!1.2 From current daytime
!------------------------
!
...
SUBROUTINE rng_normal_arr(xrand,ncount,numgen,xmean,xstd,xlo,xhi)
!************************************************************************
!
...
! Version - @(COHERENS)rng_library.f90  V2.11
!
! Description - if 'xlo' is present, the result must be larger or equal to 'xlo'
!               of 'xhi' is present, the result must be lower or equal to 'xhi'
!
...
REAL, INTENT(IN), OPTIONAL :: xhi, xlo
...
!*xlo*      REAL    If PRESENT, the result must be larger than or equal to xlo
!*xhi*      REAL    If PRESENT, the result must be lower than or equal to xhi
!
!------------------------------------------------------------------------------
!
!*Local variables
...
!
!3. Apply mean and standard deviation
!------------------------------------
!

IF (xstd.NE.1.0) xrand = xmean + xstd*xrand
...
lflag = PRESENT(xlo); hflag = PRESENT(xhi)

IF (lflag.AND.(.NOT.hflag)) THEN
   n_410: DO n=1,ncount
      DO WHILE (xrand(n).LT.xlo)
         CALL rng_standard_normal(xrand(n),numgen)
         xrand(n) = xmean + xstd*xrand(n)
      ENDDO
   ENDDO n_410
ELSEIF (.NOT.lflag.AND.hflag) THEN
   n_420: DO n=1,ncount
      DO WHILE (xrand(n).GT.xhi)
         CALL rng_standard_normal(xrand(n),numgen)
         xrand(n) = xmean + xstd*xrand(n)
      ENDDO
   ENDDO n_420
ELSEIF (lflag.AND.hflag) THEN
   n_430: DO n=1,ncount   
      DO WHILE (xrand(n).LT.xlo.OR.xrand(n).GT.xhi)
         CALL rng_standard_normal(xrand(n),numgen)
         xrand(n) = xmean + xstd*xrand(n)
      ENDDO
   ENDDO n_430
...
SUBROUTINE rng_normal_var(xrand,numgen,xmean,xstd,xlo,xhi)
!************************************************************************
!
...
IF (lflag.AND.(.NOT.hflag)) THEN
   DO WHILE (xrand.LT.xlo)
      CALL rng_standard_normal(xrand,numgen)
      xrand = xmean + xstd*xrand
   END DO
ELSEIF (.NOT.lflag.AND.hflag) THEN
   DO WHILE (xrand.GT.xhi)
      CALL rng_standard_normal(xrand,numgen)
      xrand = xmean + xstd*xrand
   ENDDO
ELSEIF (lflag.AND.hflag) THEN
   DO WHILE (xrand.LT.xlo.OR.xrand.GT.xhi)
      CALL rng_standard_normal(xrand,numgen)
      xrand = xmean + xstd*xrand
   ENDDO
ENDIF
...
FUNCTION rng_opened(numgen)
!************************************************************************
!
! *rng_opened* Returns .TRUE. if generator with index 'numgen' has been opened
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)rng_library.f90  V2.11
...
rng_opened = generator_set(numgen)


RETURN

END FUNCTION rng_opened
...
! Author -
!
! Version - @(COHERENS)rng_library.f90  V2.1.2
!
! Description -
!
! Reference - L'Ecuyer and Cote (1991)
!
! Module calls - error_abort, error_vals_var_char, rng_multmod_decompos
!
!************************************************************************
!
...
CASE ('N')
   ...
   igstate_last2(numgen) = rng_multmod_decompos(iec_a2w,igstate_last2(numgen),&
                                              & iec_m2)
CASE DEFAULT
   CALL error_vals_var_char(seedtype,'seedtype','"I" "L" "N"')
   CALL error_abort('rng_reset',ierrno_runval)
END SELECT
