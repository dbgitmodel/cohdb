...
ENDIF
!---particle module
modfiles(io_parspc,1,1)%status = 'N'
modfiles(io_parcld,1:3,1)%status = 'N'
modfiles(io_parcld,1:3,1)%endfile = 1
...
!  ---particle module
   modfiles(io_parspc,1,2)%status = 'W'
   modfiles(io_parspc,1,2)%form = 'A'
   modfiles(io_parcld,1:3,2)%status = 'W'
   modfiles(io_parcld,1:3,2)%form = 'N'
ENDIF
...
nosetstsr = MERGE(2,1,runtitle(10:10).EQ.'A')
novarstsr = MERGE(8,4,runtitle(10:10).EQ.'A')
