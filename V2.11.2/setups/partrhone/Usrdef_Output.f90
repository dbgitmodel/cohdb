...
! $Revision: 1033 $
!
! Description - output parameters for test case partrhone
!
! Reference -
!
...
!  ---particle cloud parameters
   modfiles(io_parspc,1,1)%status = 'R'
   modfiles(io_parspc,1,1)%form = modfiles(io_parspc,1,2)%form
   modfiles(io_parspc,1,1)%filename = modfiles(io_parspc,1,2)%filename
   modfiles(io_parspc,1,2)%status = '0'
!  ---particle cloud data
   modfiles(io_parcld,1:3,1)%status = 'R'
   modfiles(io_parcld,1:3,1)%form = modfiles(io_parcld,1:3,2)%form
   modfiles(io_parcld,1:3,1)%filename = modfiles(io_parcld,1:3,2)%filename
   modfiles(io_parcld,1:3,2)%status = '0'
