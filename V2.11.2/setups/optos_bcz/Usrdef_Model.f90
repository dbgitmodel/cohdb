...
modfiles(io_modgrd,1,1)%filename = 'bcz_grid.dat'
!---open boundary conditions
modfiles(io_2uvobc,1,1)%status = 'N'
modfiles(io_2uvobc,2,1)%status = 'R'
modfiles(io_2uvobc,2,1)%form = 'N'
modfiles(io_2uvobc,2,1)%filename = 'OUTDIR/'//'optnos_obc.nc'
...
IF (iopt_MPI_partit.EQ.2) THEN
   modfiles(io_mppmod,1,1)%filename = 'optbcz10.mppmod.txt'
...
surfacegrids(igrd_model,1)%x0dat = 2.077381
surfacegrids(igrd_model,1)%y0dat = 50.99653
surfacegrids(igrd_model,1)%delxdat = 0.1190476E-01
surfacegrids(igrd_model,1)%delydat = 0.6944444E-02
...
j_210: DO j=1,nr-1
   READ (iunit,9001) depmeanglb(1:nc-1,j)
ENDDO j_210
...
READ (iunit,'(//)')
ii_310: DO ii=1,nobu
   READ (iunit,9002) iobu(ii), jobu(ii)
ENDDO ii_310

!---V-nodes
READ (iunit,*)
jj_320: DO jj=1,nobv
   READ (iunit,9002) iobv(jj), jobv(jj)
ENDDO jj_320
