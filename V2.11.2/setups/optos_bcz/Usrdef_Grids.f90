...
!---regular grid
delxdat = 1/84.0; delydat = 1/144.0
surfacegrids(igrd_model,1)%delxdat = delxdat
surfacegrids(igrd_model,1)%delydat = delydat
surfacegrids(igrd_model,1)%x0dat = 2.0 + 13/168.0
surfacegrids(igrd_model,1)%y0dat = 51.0 - 0.5*delydat
...
CALL open_file(iunit,'bcz_grid.dat','IN','A')
READ (iunit,'(//)')
...
j_210: DO j=1,nr-1
   READ (iunit,9001) depmeanatc(1:nc-1,j)
ENDDO j_210
