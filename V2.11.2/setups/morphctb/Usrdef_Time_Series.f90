...
IF (iopt_sed_mode.EQ.4) THEN
   tsrvars%ivarid = (/iarr_zeta,iarr_umvel,iarr_vmvel,iarr_bed_update,&
                    & iarr_bed_update_int,iarr_bstresatc,iarr_qtotatu,&
                    & iarr_uvel,iarr_vvel,iarr_wphys/)
   tsrvars(1:7)%nrank = 2; tsrvars(8:10)%nrank = 3
   tsrvars(7)%numvar = 1
ELSEIF (iopt_morph_fixed_layer.EQ.0) THEN
   tsrvars%ivarid = (/iarr_zeta,iarr_umvel,iarr_vmvel,iarr_bed_update,&
                    & iarr_bed_update_int,iarr_bstresatc,iarr_qbedatu,&
                    & iarr_bottom_sed_flux,iarr_uvel,iarr_vvel,iarr_wphys,&
                    & iarr_ctot/)
   tsrvars(1:8)%nrank = 2; tsrvars(9:12)%nrank = 3
   tsrvars(7:8)%numvar = 1
ELSE
   tsrvars%ivarid = (/iarr_zeta,iarr_umvel,iarr_vmvel,iarr_bed_update,&
                    & iarr_bed_update_int,iarr_bstresatc,iarr_qbedatu,&
                    & iarr_bottom_sed_flux,iarr_sed_avail_tot,iarr_uvel,&
                    & iarr_vvel,iarr_wphys,iarr_ctot/)
   tsrvars(1:9)%nrank = 2; tsrvars(10:13)%nrank = 3
   tsrvars(7:8)%numvar = 1
ENDIF
