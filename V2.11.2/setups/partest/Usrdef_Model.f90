...
SELECT CASE (runtitle(8:8))
   CASE ('A','B','C','D','E')
      iopt_part_model = MERGE(1,2,npworld.EQ.1)
   CASE ('F'); iopt_part_model = 3
   CASE ('G'); iopt_part_model = 4
END SELECT
...
!---particle model
SELECT CASE (runtitle(8:8))
   CASE ('A','B'); iopt_part_write = 1
   CASE DEFAULT; iopt_part_write = 0
END SELECT

!---user output
SELECT CASE (runtitle(8:8))
   CASE ('D','E'); iopt_out_tsers = 1
   CASE DEFAULT; iopt_out_tsers = 0
END SELECT
...
SELECT CASE (iopt_part_model)
   CASE (1,2)
      CStartDateTime = '2003/01/01;00:00:00:000'
      CEndDateTime = '2003/01/03;00:00:00:000'
   CASE (3)
      CStartDateTime = '2003/01/02;00:00:00:000'
      CEndDateTime = '2003/01/03;00:00:00:000'
   CASE (4)
      CStartDateTime = '2003/01/03;00:00:00:000'
      CEndDateTime = '2003/01/02;00:00:00:000'
END SELECT

!---time step
delt2d = MERGE(3.0,30.0,iopt_part_model.LE.2)  

!---counter for 3-D mode
ic3d = MERGE(10,1,iopt_part_model.LE.2)
...
!---initial conditions
IF (runtitle(8:8).LT.'G') THEN
   modfiles(io_inicon,ics_part,1)%status = 'N'
ELSE
   modfiles(io_inicon,ics_part,1)%status = 'R'
   modfiles(io_inicon,ics_part,1)%filename = 'partest.parfin.nc'
   modfiles(io_inicon,ics_part,1)%form = 'N'
ENDIF
...
!---output
IF (runtitle(8:8).EQ.'A') THEN
   norestarts = 1
   ntrestart(1) = int_fill
   modfiles(io_fincon,ics_part,2)%status = 'W'
   modfiles(io_fincon,ics_part,2)%form = 'N'
   modfiles(io_fincon,ics_part,2)%filename = 'partest.parfin.nc'
ELSEIF (runtitle(8:8).NE.'G'.AND.ciffile%status.EQ.'W') THEN
   norestarts = 1
   ntrestart(1) = 0
   modfiles(io_fincon,ics_part,2)%status = 'W'
   modfiles(io_fincon,ics_part,2)%form = 'N'
...
IF (iopt_part_write.EQ.1) THEN
   ...
   icount = MERGE(10,40,runtitle(8:8).EQ.'A')
   modfiles(io_parphs,1,2)%tlims = (/28800,int_fill,icount/)
ENDIF
...
IF (iopt_part_model.GE.3) THEN
   ...
   IF (iopt_part_model.EQ.3) THEN
      modfiles(io_pargrd,1,1)%filename = 'partestB.pargrd.nc'
      modfiles(io_parphs,1,1)%filename = 'partestB.parphs.nc'
   ELSE
      modfiles(io_pargrd,1,1)%filename = 'partestA.pargrd.nc'
      modfiles(io_parphs,1,1)%filename = 'partestA.parphs.nc'
   ENDIF
ENDIF
