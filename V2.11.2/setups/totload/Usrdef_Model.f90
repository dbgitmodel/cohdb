...
!************************************************************************
!
! *Usrdef_Model* User-defined model for totalload test
!
...
!    totloadA: Engelund-Hansen (1967)
!    totloadB: Engelund-Hansen (1967), Cunge form
!    totloadC: Ackers White (1973)
!    totloadD: Wu et al (2000)
!    totloadE: Van Rijn (2003)
!    totloadF: Van Rijn (2007)
!
! Reference -
!
...
IF (nt.LT.nstep) THEN
   ...
   vdvel_cst = 0.2*depmean_cst*icount
   WHERE (node2dv(1:ncloc,1:nrloc).GT.1)
      vdvel(1:ncloc,1:nrloc) = vdvel_cst
      vmvel(1:ncloc,1:nrloc) = vdvel_cst/depmean_cst
      vvel(1:ncloc,1:nrloc,1) = vmvel(1:ncloc,1:nrloc)
   END WHERE
ENDIF
