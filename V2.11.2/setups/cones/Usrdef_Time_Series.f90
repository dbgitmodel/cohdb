...
! Author - Patrick Luyten
!
! Version - @(COHERENS)Usrdef_Time_Series.f90  V2.1.1
!
! Description - test case cones
!
! Reference -
...
IF (iopt_verif.EQ.0) THEN
   tsrvars%ivarid = (/iarr_uvel,iarr_vvel,iarr_wphys,iarr_sal/)
   tsrvars%nrank = 3
ELSE
...
tsr2d(1)%defined = MERGE(.FALSE.,.TRUE.,iopt_verif.EQ.0)
tsr3d(1)%defined = .TRUE.
tsr2d(1)%form = MERGE('A','N',iopt_CDF.EQ.0)
tsr3d(1)%form = MERGE('A','N',iopt_CDF.EQ.0)
...
tsrgpars(1)%zlims = (/1,1,1/)
tsrgpars(1)%tlims = (/0,nstep,42/)
