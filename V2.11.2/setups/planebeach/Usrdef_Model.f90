...
! $Revision: 1130 $
!
! Description - test case planebeach
!
! Reference -
!
...
iopt_grid_nodim = MERGE(3,2,cexp.EQ.'F')

!---bottom stress formulation (2/3)
...
iopt_waves = 1
iopt_waves_curr = 1
iopt_waves_dissip = MERGE(0,1,cexp.EQ.'B')
iopt_waves_pres = MERGE (0,1,cexp.EQ.'C')
iopt_waves_form = MERGE(1,2,cexp.EQ.'D')
IF (npworld.EQ.1) THEN
   iopt_waves_couple = 0
ELSE
   iopt_waves_couple = MERGE(1,3,cexp.EQ.'E')
ENDIF
...
CEndDateTime(1:19)   = '2004/12/24;01:00:00'
...
modfiles(io_modgrd,1,1)%filename = 'PB_COHERENS.dep'
...
IF (iopt_waves_couple.EQ.0) THEN
   ...
   modfiles(io_wavsur,1,1)%tlims = (/0,int_fill,300/)
ELSE
...
!
!---model
delxdat = 20.0; delydat = 20.0
surfacegrids(igrd_model,1)%x0dat = 0.0-delxdat/2
surfacegrids(igrd_model,1)%y0dat = 0.0-delydat/2
