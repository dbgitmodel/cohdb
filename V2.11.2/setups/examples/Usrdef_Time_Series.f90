...
!************************************************************************
!
! *Usrdef_Time_Series* Define time series output (example routines)
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)Usrdef_Time_Series.f90  V2.1.2
!
! $Date: 2013-08-08 15:42:33 +0200 (Thu, 08 Aug 2013) $
!
! $Revision: 609 $
!
! Description -
...
!************************************************************************
!
! *usrdef_tsr_params* Specifiers for time series output (example routine)
!
! Author - Patrick Luyten
!
...
iset_310: DO iset=1,nosetstsr

!  ---defined
   tsr0d(iset)%defined = .FALSE.; tsr2d(iset)%defined = .FALSE.
   tsr3d(iset)%defined = .FALSE.; tsrgrd(iset)%defined = .FALSE.

!  ---file format ('A','U','N')
   tsr0d(iset)%form = 'U'; tsr2d(iset)%form = 'U'
   tsr3d(iset)%form = 'U'; tsrgrd(iset)%form = 'U'

!  ---file name
   tsr0d(iset)%filename = ''; tsr2d(iset)%filename = ''
   tsr3d(iset)%filename = ''; tsrgrd(iset)%filename = ''

!  ---info file
   tsr0d(iset)%info = .TRUE.; tsr2d(iset)%info = .TRUE.
   tsr3d(iset)%info = .TRUE.; tsrgrd(iset)%info = .TRUE.
...
istat_710: DO istat=1,nostatstsr
!  ---X-index
   tsrstatlocs(istat)%ipos = ?
!  ---Y-index
   tsrstatlocs(istat)%jpos = ?
!  ---station name
   tsrstatlocs(istat)%name = ?
ENDDO istat_710
...
iset_610: DO iset=1,nosetstsr
   nostats = tsrgpars(iset)%nostats
   lstatstsr(iset,1:nostats) = ?
ENDDO iset_610
...
!************************************************************************
!
! *usrdef_tsr0d_vals* 0-D output data (example routine)
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)Usrdef_Time_Series.f90  V2.1.2
...
!************************************************************************
!
! *usrdef_tsr2d_vals* 2-D output data (example routine)
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)Usrdef_Time_Series.f90  V2.1.2
...
!*Arguments
!
INTEGER, INTENT(IN) :: i, j, k, n2vars
REAL, INTENT(OUT), DIMENSION(n3vars) :: out2ddat
...
!************************************************************************
!
! *usrdef_tsr3d_vals* 3-D output data (example routine)
!
! Author - Patrick Luyten
!
! Version - @(COHERENS)Usrdef_Time_Series.f90  V2.1.2
